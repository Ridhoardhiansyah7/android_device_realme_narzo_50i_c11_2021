...
int GUIAction::flashlight(std::string arg __unused)
{
	...
	if (TWFunc::Path_Exists(flashpath)) {
		...
		if (DataManager::GetIntValue("pb_torch_on") == 1)
		{
			...
			TWFunc::write_to_file(flashpath, "1");   // ← CHANGED
		...
		else
		{
			...
			TWFunc::write_to_file(flashpath, std::to_string(br_value < 2 ? 2 : br_value)); // ← CHANGED
			if (TWFunc::Path_Exists(switch_path))
				TWFunc::write_to_file(switch_path + bright, "2"); // ← CHANGED
